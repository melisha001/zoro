...
export default function AdminDoubtsPage() {
  ...
  const loadDoubts = async () => {
    ...
    try {
      ...
      console.error('Failed to load doubts for admin:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleReply = async (id) => {
    ...
    try {
      const res = await resolveDoubt(id, 'Admin', text);
      if (res.success) {
  ...
  const filteredDoubts = (activeTab === 'open' ? openDoubts : answeredDoubts).filter(d => {
    const sName = d.studentName?.toLowerCase() || '';
    const qText = d.question?.toLowerCase() || '';
    const query = searchTerm.toLowerCase();
    return sName.includes(query) || qText.includes(query);
  });
            ...
            <div key={doubt.id} className="bg-white rounded-2xl border border-slate-200 p-5 shadow-xs space-y-4">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-slate-900 text-sm">{doubt.studentName}</h3>
                    <p className="text-xs text-slate-500 font-medium">
                      Course: <span className="font-bold text-[#0F52BA]">{doubt.course}</span> • Assigned Trainer: <span className="font-semibold text-slate-700">{doubt.trainerName || 'Unassigned'}</span>
                    </p>
                  </div>
                </div>
              ...
              {doubt.status?.toUpperCase() === 'OPEN' ? (
                ...
                <div className="bg-blue-50/50 p-4 rounded-xl border border-blue-100 text-slate-800 text-xs space-y-1">
                  <span className="text-[11px] font-bold text-[#0F52BA]">Answered by {doubt.trainerName || 'Admin'}:</span>
